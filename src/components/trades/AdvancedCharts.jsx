...
export default function AdvancedCharts({ trades, initialBalance }) {
  const operationsData = trades.map((trade, idx) => ({
    ...
    [cite_start]impacto: ((trade.resultado / initialBalance) * 100).toFixed(2), [cite: 22]
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="p-4 bg-white dark:bg-slate-800 rounded-xl shadow">
        [cite_start]<h3 className="text-lg font-bold mb-4">Resultado por Operação ($)</h3> [cite: 22]
      ...
      <div className="p-4 bg-white dark:bg-slate-800 rounded-xl shadow">
        [cite_start]<h3 className="text-lg font-bold mb-4">Impacto no Saldo (%)</h3> [cite: 22]
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={operationsData}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="index" />
